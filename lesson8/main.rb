...
class Main
  ...
  def train_move_forward(train)
    if train.move_forward
      sended_from = "Поезд отправлен со станции #{train.previous_station}"
      arrived_at = "Поезд прибыл на станцию #{train.current_station}"
      [sended_from, arrived_at].join('\n')
    else
  ...
  def train_move_backward(train)
    if train.move_backward
      sended_from = "Поезд отправлен со станции #{train.next_station}"
      arrived_at = "Поезд прибыл на станцию #{train.current_station}"
      [sended_from, arrived_at].join('\n')
    else
